# =============================================================================
# 03_h2o_train.R — H2O AutoML Training Pipeline
#
# Description: This script trains habitat suitability models using H2O AutoML.
# It automatically selects the best ensemble architecture (GBM, DRF, etc.)
# for each elephant and generates spatial predictions and variable importance.
# =============================================================================
...
res_root <- cfg$paths$results_h2o %||% file.path(.root, "results", "H2O")
...
for (sp in sp_list) {
  ...
  bg_pts <- terra::spatSample(Renv[[1]],
    size = nrow(pres), method = "random",
  ...
  if (length(cls_tab) < 2 || any(cls_tab < 10)) {
    log_line(sprintf(
      "Skip %s — class imbalance after drop_na() (counts: %s)", sp,
